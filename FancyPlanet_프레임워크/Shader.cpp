...
D3D12_INPUT_LAYOUT_DESC CDefferredTexturedShader::CreateInputLayout()
{
	...
	pd3dInputElementDescs[2] = { "WEIGHTS", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 20, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[3] = { "BONEINDICES", 0, DXGI_FORMAT_R32G32B32A32_UINT, 0, 32, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
...
void CTextureToFullScreenShader::UpdateMaterial(CGameObject *pObject) 
{
	...
	if (pObject->m_pSibling)
		UpdateMaterial(pObject->m_pSibling);
	if (pObject->m_pChild)
		UpdateMaterial(pObject->m_pChild);
}
...
void CTextureToFullScreenShader::BuildLightsAndMaterials()
{
	...
	m_pLights->m_pLights[2].m_xmf4Diffuse = XMFLOAT4(0.05f, 0.05f, 0.05f, 1.0f);
	m_pLights->m_pLights[2].m_xmf4Specular = XMFLOAT4(0.2f, 0.2f, 0.2f, 1.0f);
	m_pLights->m_pLights[2].m_xmf3Direction = XMFLOAT3(-1.0f, -1.0f, -1.0f);
...
D3D12_DEPTH_STENCIL_DESC CSkyBoxShader::CreateDepthStencilState()
{
	...
	d3dDepthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO; // wirte을 하지 않게 한다.
